"""
Agentic On-Policy Distillation Training Script for RTL Code Generation - Qwen Version
...
This version uses Qwen models with standard tinker renderers.
"""
...
from cvdp_agentic_dataset import CVDPAgenticDatasetBuilderQwen
...
class CLIConfig:
    ...
    lora_rank: int = 32
    renderer_name: str | None = None  # Will auto-detect based on model
    load_checkpoint_path: str | None = None

    # Dataset configuration
    cvdp_jsonl_path: str = "cvdp_1_problem.jsonl"
...
async def main(cli_config: CLIConfig):
    ...
    # Get renderer name - auto-detect based on model or use override
    if cli_config.renderer_name:
        renderer_name = cli_config.renderer_name
    else:
        # Get recommended renderer for the model
        renderer_name = model_info.get_recommended_renderer_name(cli_config.student_model)
    ...
    if cli_config.log_path is not None:
        ...
        teacher_name = cli_config.teacher_model.replace("/", "-").split("-")[-1]  # e.g., "32B"
        run_name = (
            f"agentic-qwen-distill-{student_name}-from-{teacher_name}-"
        ...
        log_path = os.path.expanduser(f"~/tinker-examples/rtl-agentic-qwen/{run_name}")
    ...
    logger.info("=" * 80)
    logger.info("RTL Code Generation - Agentic On-Policy Distillation (Qwen)")
    logger.info("=" * 80)
    ...
    dataset_builder = CVDPAgenticDatasetBuilderQwen(
